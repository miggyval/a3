...
class EnergyTower(AbstractTower):
    """A long range energy tower that rotates towards enemies and shoots projectiles at them"""
    name = 'Energy Tower'
    colour = '#CCCCFF'  # Periwinkle
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9), rotation=math.pi * .25, base_damage=15, level: int = 1):
        super().__init__(cell_size, grid_size, rotation, base_damage, level)
        self.roman = None
...
class GlueTower(AbstractTower):
    """A medium range tower that shoots glue at enemies to slow them down. Does no damage"""
    name = 'Glue Tower'
    colour = '#FFC40C'  # Mikado
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9), rotation=math.pi * .25, base_damage=0, level: int = 1):
        super().__init__(cell_size, grid_size, rotation, base_damage, level)
        self.roman = None
...
class LaserTower(SimpleTower):
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9), rotation=math.pi * .25, base_damage=20, level: int = 1):
    ...
    def step(self, units):
        ...
        # Main damage
        if self._target is not self._previous_target and not self.is_upgrading:
            if self._laser_beam is not None:
                self._laser_beam.is_off = True
            self.cool_down.start()
            self._laser_beam = LaserBeam(self.position, self.cell_size, self._target, damage=self.get_damage())
            self._laser_beam.target = self._target
            target.damage(self.get_damage(), type_='energy')
            x1, y1 = self.position
            x2, y2 = target.position
            d = int(math.ceil(math.sqrt((x2-x1) ** 2 + (y2-y1) ** 2)))
            point_list = []
            beam_angle = math.atan2(y2 - y1, x2 - x1)
            self.rotation = beam_angle
            for i in range(0, (d//20)+1):
                point_list.append((x1 + 20*i*math.cos(beam_angle), y1 + 20*i*math.sin(beam_angle)))
            point_list.append(point_list[0])
            target_list = []
            for point in point_list:
                for near_target in self.get_units_in_range(units.enemies):
                    xt, yt = near_target.position
                    xk, yk = point
                    if math.sqrt((xt - xk) ** 2 + (yt-yk) ** 2) < 2:
                        if target not in target_list:
                            target_list.append(near_target)

            for other_target in target_list:
                other_target.damage(math.floor(self.get_damage() * 20), type_='energy')
            self._previous_target = self._target
            return [self._laser_beam]


class LaserBeam(AbstractObstacle):
    """A simple projectile fired from a MissileTower"""
    name = "Laser Beam"
    colour = '#F5F000'  # Eburnean

    rotation_threshold = (1 / 3) * math.pi

    def __init__(self, position, cell_size, target: AbstractEnemy, size=.2,
                 rotation: Union[int, float] = 0, grid_speed=0, damage=10):
    ...
    def step(self, units):
        ...
        Return:
            (persist, new_obstacles) pair, where:
                ...
                - new_obstacles (list[AbstractObstacle]): A list of new obstacles to add to the game, or None
        """
        self.target.damage(self.damage, type_='Energy')
        if self.target.is_dead() or self.is_off:
            return False, None
        return True, None
...
class MissileTower(SimpleTower):
    ...
    def __init__(self, cell_size: int, grid_size=(.9, .9), rotation=math.pi * .25, base_damage=100, level: int = 1):
        super().__init__(cell_size, grid_size=grid_size, rotation=rotation, base_damage=base_damage, level=level)
        self.roman = None
        self._target: AbstractEnemy = None
