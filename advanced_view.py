...
#                             You Don't,
#                           So Look Again
#                  (JOKES ON YOU BEN I CAN DO THIS)
#
...
from tower import AbstractTower, MissileTower, PulseTower, SimpleTower, LaserTower, LaserBeam, \
...
class TowerView(SimpleView):
    ...
    @classmethod
    def _draw_laser(cls, canvas: tk.Canvas, tower_: MissileTower):
        """Draws a laser tower"""
...
class EnemyView(SimpleView):
    ...
    def _draw_simple(cls, canvas: tk.Canvas, enemy: AbstractEnemy):
        ...
        if extent == 360:  # because tkinter is lame (EDITOR'S COMMENT: "Yes. Yes it is.")
            extent = 359.9999
...
class ObstacleView(SimpleView):
    ...
    def _draw_missile(cls, canvas: tk.Canvas, missile: Missile):
        ...
        head = x + 1.8*dx, y + 1.8*dy
        tail = x - dx, y - dy
        point1 = x - 0.8*dx + 0.5*dy, y - 0.8*dy - 0.5*dx
        point2 = x - 0.8*dx - 0.5*dy, y - 0.8*dy + 0.5*dx
        point3 = x + 0.8*dx + 0.5*dy, y + 0.8*dy - 0.5*dx
        point4 = x + 0.8*dx - 0.5*dy, y + 0.8*dy + 0.5*dx
        point5 = x - 2*dx + 1.5*dy, y - 2*dy - 1.5*dx
        point6 = x - 2*dx - 1.5*dy, y - 2*dy + 1.5*dx
        canvas.create_polygon(point1, point2, point4, head, point3, point1, tags=['obstacle', 'rocket'], outline='black',
                              fill='white')
        return canvas.create_polygon(point1, point2, point6, point5, point1, fill='red', tags=['obstacle', 'rocket'])

    @classmethod
    def _draw_laser_beam(cls, canvas: tk.Canvas, missile: Missile):
        """Draws a missile"""
        return canvas.create_line(missile.origin, missile.target.position, width=5, fill='red', tags=['obstacle', 'laser'])
